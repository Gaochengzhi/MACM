...
% s.id = 1;
% s.method = "notice-event";
% s.content.arguments.vesPostion = 1;
...
while sum(cell2mat(p_GCAA))~=0 %%程序运行直到静目标全部找到为止
    ...
    pos_a_points = X(1:2,:,3)';

    pos_a_loop_rec(:,1) = pos_a_loop(:,1) + deviation_x;
    pos_a_loop_rec(:,2) = pos_a_loop(:,2) + deviation_y;
    pos_a_loop3 = zeros(size(pos_a_loop_rec,1),3);
    pos_a_loop3(:,1:2) = pos_a_loop_rec;
    pos_a_loop3(:,3) =  3.9765e+06;
    pos_a_lla = ecef2lla(pos_a_loop3, 'WGS84');
    pos_a_output = pos_a_lla(:,[2,1]);

    pos_a_points(:,1) = pos_a_points(:,1) + deviation_x;
    pos_a_points(:,2) = pos_a_points(:,2) + deviation_y;
    pos_a_points3 = zeros(size(pos_a_points,1),3);
    pos_a_points3(:,1:2) = pos_a_points;
    pos_a_points3(:,3) =  3.9765e+06;
    pos_a_points_lla = ecef2lla(pos_a_points3, 'WGS84');
    pos_a_points_output = pos_a_points_lla(:,[2,1]);
    ...
    spd_temp = X(1:2,:,2) - X(1:2,:,1);
    for j = 1:size(spd_temp,2)
        spd(j) = norm(spd_temp(:,j))/(time_step*0.51444);
        course(j) = atan2(spd_temp(2,j),spd_temp(1,j))*180/pi;
        if course(j) > 0
            course(j) = 450 - course(j);
        else
            course(j) = abs(course(j)) + 90;
        end
    end

    points_temp = X(1:2,:,3) - X(1:2,:,2);
    for j = 1:size(points_temp,2)
        spd_points(j) = norm(points_temp(:,j))/(time_step*0.51444);
    end


    for i = 1:size(agent_name,2)
        curr_pos = struct('coord',pos_a_output(i,:),'spd',spd(i),'course',course(i));
        s.content.arguments.vesPostion.(agent_name{1,i}) = curr_pos;

        points = [struct('coord',pos_a_points_output(i,:),'spd',spd_points(i))];
        waypoint = struct('shape','LineString','points',points);
        id = [id struct('id',(agent_name{1,i}),'path',waypoint)];
        s.content.arguments.road = id;
    end

    t = jsonencode(s,'PrettyPrint',true);
    % file= fopen(sprintf('%05d.json',i), 'w+');
    file= fopen(fullfile(cd,'stage1_json',sprintf('%05d.json',i_round)), 'w+');
    fprintf(file, '%s',t);
    fclose(file);
...
for i_round=1:n_rounds
    ...
    for j = 1:na
        if size(Xfinal{j},2) >= 3
            pos_a_points(j,:) = Xfinal{j}(1:2,3)';
            points_temp(:,j) = Xfinal{j,1}(1:2,3)' - Xfinal{j,1}(1:2,2)';
        elseif ~isempty(Xfinal{j})
            pos_a_points(j,:) = Xfinal{j}(1:2,size(Xfinal{j},2))';
            points_temp(:,j) = Xfinal{j,1}(1:2,size(Xfinal{j},2))' - Xfinal{j,1}(1:2,size(Xfinal{j},2))';
        end
    end
    ...
    for j = 1:na
        if size(Xfinal{j},2) >= 2
        spd_temp = Xfinal{j,1}(1:2,2) - Xfinal{j,1}(1:2,1);
        elseif ~isempty(Xfinal{j})
            spd_temp = Xfinal{j,1}(1:2,size(Xfinal{j},2))' - Xfinal{j,1}(1:2,size(Xfinal{j},2))';
        end
        spd(j) = norm(spd_temp)/(time_step*0.51444);
        if sum(spd_temp)~=0
        course(j) = atan2(spd_temp(2,1),spd_temp(1,1))*180/pi;
        end
        if course(j) > 90 && course(j) <180
            course(j) = 450 - course(j);
        else
            course(j) = 90 - course(j);
        end
    end
    ...
    t = jsonencode(s,'PrettyPrint',true);
    % file= fopen(sprintf('%05d.json',i), 'w+');
    file= fopen(fullfile(cd,'stage2_json',sprintf('%05d.json',i_round)), 'w+');
    fprintf(file, '%s',t);
    fclose(file);
