...
def gcaa_solution(agents, G, tasks_cells):
    ...
    # Run GCAA
    GCAA_Assignments, S_GCAA_agents, S_GCAA_ALL_agents, agents_list = gcaa_main(agents_list, tasks_list, G, tasks_cells['prob_a_t'], tasks_cells['lambda'])
    ...
    rt = np.zeros(nt)
    for j in range(nt):
        S_GCAA_ALL[j] = calc_task_utility(agents['Pos'], agents['v_a'], tasks_cells['Pos'][j], tasks_cells['Speed'][j], tasks_cells['tf'][j], tasks_cells['r_bar'][j], j+1, tasks_cells['prob_a_t'], winners_matrix, tasks_cells['lambda'], agents['kdrag'])
        rt[j] = tasks_cells['r_bar'][j] * (1 - np.prod(1 - winners_matrix[:, j] * tasks_cells['prob_a_t'][:, j]))
    ...
    for i in range(na):
        ...
        if len(task_idx) == 0:
            ...
            task_idx = task_idx[0]
            if tasks_list[task_idx]['tloiter'] > 0 and (tasks_list[task_idx]['tf'] - tasks_list[task_idx]['tloiter']) / tasks_list[task_idx]['tloiter'] < 1:
