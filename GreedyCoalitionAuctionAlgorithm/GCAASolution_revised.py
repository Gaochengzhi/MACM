...
def  GCAASolution_revised(Agents, G, TasksCells):
    ...
    rt = np.zeros(nt)
    for j in range(nt):
        S_GCAA_ALL[j] = calc_task_utility(Agents['Pos'], Agents['v_a'], TasksCells['Pos'][j, :2], TasksCells['Speed'][j], TasksCells['tf'][j], TasksCells['r_bar'][j], j + 1, TasksCells['prob_a_t'], winners_matrix, TasksCells['lambda'], Agents['kdrag'])
        rt[j] = TasksCells['r_bar'][j] * (1 - np.prod(1 - winners_matrix[:, j] * TasksCells['prob_a_t'][:, j]))
    ...
    for i in range(na):
        ...
        if len(task_idx) == 0:
            ...
            task_idx = task_idx[0]
            if tasks[task_idx]['tloiter'] > 0 and (tasks[task_idx]['tf'] - tasks[task_idx]['tloiter']) / tasks[task_idx]['tloiter'] < 1:
