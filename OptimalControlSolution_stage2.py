...
def OptimalControlSolution_stage2(Agents, Tasks, p_GCAA, time_step, n_rounds, kdrag):
    ...
    for i in range(na):
        ...
        for j in range(1):
            ...
            # here is a call to a function not defined in your provided code
            ...
            while np.linalg.norm(pos_t_curr - X[0:2, i, k]) > 53:
                ...
                if (
                    ...
                    d = np.linalg.norm(r_target_circle)
                    curr_velocity = np.zeros_like(X[:, i, k + 1])
                    curr_velocity[0:2] = r_target_circle / d
                    X[:, i, k + 1] = X[:, i, k] + time_step * curr_velocity * 500
                    tmp = k + 1
        ...
        if flag_changeTargets[ind_task] <= 1 and targets_restCheck[ind_task] <= 1:
            ...
            len = k - tmp + 1
            Xtmp = X[:, i, tmp:k]
            Xtmp1 = np.flip(Xtmp[:, :, : len - 1], axis=2)

            X[:, i, k + 1 : k + 1 + len - 2] = Xtmp1
            ...
            len = k - tmp + 1
            Xtmp = X[:, i, tmp - 1 : k]
            Xtmp1 = np.flip(Xtmp[:, :, :len])
            Xtmp2 = np.concatenate((Xtmp1, Xtmp[:, :, 1:len]), axis=2)
            Xtmp3 = np.tile(Xtmp2, (1, 1, targets_restCheck[ind_task] - 1))

            X[
                :, i, k + 2 : k + (targets_restCheck[ind_task] - 1) * (2 * len - 2)
            ] = Xtmp3
