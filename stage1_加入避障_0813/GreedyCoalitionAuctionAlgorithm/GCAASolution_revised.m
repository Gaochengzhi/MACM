...
function [S_GCAA, p,taskInd, S_GCAA_ALL, rt, Agents] = GCAASolution(Agents, G, TasksCells)
    ...
    rt         = zeros(1,nt);
    for j = 1:nt
        S_GCAA_ALL(j) = CalcTaskUtility(Agents.Pos, Agents.v_a, TasksCells.Pos(j,1:2), TasksCells.Speed(j,:), TasksCells.tf(j), TasksCells.r_bar(j), j, TasksCells.prob_a_t, winners_matrix, TasksCells.lambda, Agents.kdrag);
